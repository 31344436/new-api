...
const PRIMARY_MODEL_OPTION_KEYS = [
  'ModelPrice',
  'ModelRatio',
  'CompletionRatio',
  'CompletionRatioMeta',
];
const AUXILIARY_MODEL_OPTION_KEYS = [
  'CacheRatio',
  'CreateCacheRatio',
  'ImageRatio',
  'AudioRatio',
...
const collectModelNames = (
  sourceMaps,
  candidateModelNames,
  restrictToCandidateModels,
) => {
  const names = new Set(candidateModelNames);
  const optionKeys =
    restrictToCandidateModels
      ? PRIMARY_MODEL_OPTION_KEYS
      : [...PRIMARY_MODEL_OPTION_KEYS, ...AUXILIARY_MODEL_OPTION_KEYS];

  optionKeys.forEach((key) => {
    Object.keys(sourceMaps[key] || {}).forEach((name) => {
      names.add(name);
    });
  });
...
export function useModelPricingEditorState({
  ...
  restrictToCandidateModels = false,
  filterMode = 'all',
}) {
  ...
  }, [candidateModelNames, filterMode, options, restrictToCandidateModels]);
